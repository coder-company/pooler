...
| Performance | Three consecutive documented 1 MiB benchmark runs meet opaque p95 < 2 ms and semantic p95 < 5 ms. | Passed for implementation commit `9313fa1`; see `docs/benchmark-evidence-2026-08-20.md`. A later release candidate must rerun it unchanged. |
| Stress | Reproducible 15-minute mixed-protocol run processes at least 10,000 requests at 100 clients with 20% deterministic failures, drains cleanly, and meets RSS budget. | Passed with 875,400 requests and all invariants true for `9313fa1`; later commits are not covered. |
