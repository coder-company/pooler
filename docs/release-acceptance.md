...
Every workflow job targets an explicit Blacksmith runner class: Linux x86_64 uses
...
| Security | Secret-redaction, owner-only storage, cancellation, dependency, license, and vulnerability gates pass. | Local root/fuzz audit and deny gates pass; Blacksmith CI persists supply-chain logs for every pushed commit. |
| Performance | Three consecutive 1 MiB benchmark runs meet opaque p95 < 2 ms and semantic p95 < 5 ms. | Run the enforced benchmark against the exact clean release candidate; historical results never satisfy a new release. |
| Stress | Reproducible 15-minute mixed-protocol run processes at least 10,000 requests at 100 clients with 20% deterministic failures, drains cleanly, and meets the RSS budget. | The enforced benchmark records the exact candidate, workload, resource accounting, and invariant results. |
| Artifacts | Linux x86_64/ARM64 and macOS ARM64/x86_64 binaries, checksums, signatures, SBOM, and provenance are published. | The hosted release workflow must produce and validate every target from the tagged candidate. |
