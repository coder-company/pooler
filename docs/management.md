...
Account enable, disable, and switch operations commit durable credential state before publishing one coordinated update to every live runtime generation. Both policy-based pools and policy-free model routes honor that enablement. A switch enables the selected account and disables its same-provider siblings in one SQLite transaction, while accounts for other providers remain unchanged. OAuth refresh, revoke, and documented device-login requests enter the bounded native-runtime command queue and return `202 Accepted`; their eventual result is written to the audit view. Device polling runs independently so it cannot starve refresh, revoke, or reload controls. Revocation removes only Pooler's local credential payload and disables the account. It does not claim provider-side revocation unless the provider flow explicitly performs it.
