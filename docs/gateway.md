...
Errors generated by Pooler on inference listeners use the OpenAI-compatible
JSON envelope `{"error":{"message", "type", "param", "code"}}` with
`Content-Type: application/json` and `Cache-Control: no-store`. Authentication
failures retain `WWW-Authenticate: Bearer`. Opaque upstream error statuses,
headers, and bodies remain provider responses and are not rewritten.
