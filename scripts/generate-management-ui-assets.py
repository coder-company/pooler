...
def write_providers_js(registry: dict[str, dict[str, str]]) -> Path:
    ...
    lines = [
        ...
        '  return `<svg class="provider-logo" viewBox="${entry.viewBox}" width="${size}" height="${size}" fill="currentColor" role="img" aria-label="${slug}">${entry.body}</svg>`;',
        "}",
        "",
        ...
        '  return `<span class="provider-monogram" style="width:${size}px;height:${size}px;font-size:${Math.max(8, Math.round(size * 0.52))}px" aria-hidden="true">${letter}</span>`;',
        "}",
        "",
    ]
